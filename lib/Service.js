...
Service.prototype._sendSOAPActionRequest = function (device, url, serviceType, action, inArguments, outArguments, vars, callback) {
    ...
    if (false && device._sessionId) {
        var idx = head.indexOf('</s:Header>');
        if (idx >=0) {
            head = head.substr(0, idx) +
                '<dslf:SessionID xmlns:dslf="dslforum-org" s:mustUnderstand="1">' + device._sessionId +
                '</dslf:SessionID>' +
                '</s:Header>';
        }
    }
    ...
    request({
        ...
        if (!error && response.statusCode == 200) {
            ...
            error = new Error("sendSOAPActionRequest Error action=" + action + ' serviceType=' + serviceType + ": " + (response ? response.statusCode : ""));
            error.code = response ? response.statusCode : "";
            //error.response = response;
            callback(error, null);
            //  console.log('error: '+ response.statusCode)
            console.log("tr-064.service.sendSOAPActionRequest Error: action=" + action + ' serviceType=' + serviceType);
